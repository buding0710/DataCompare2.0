...
class CompareDatabaseData:
    ...
    def fullTableMD5compare(self,get_data_sql1, get_data_sql2,get_primary_key_sql1, get_primary_key_sql2,get_indexes_sql1, get_indexes_sql2,get_columns_names_sql1,
        ...
        table1_md5 = hashlib.md5(str(table1_data).encode('utf8')).hexdigest()
        table2_md5 = hashlib.md5(str(table2_data).encode('utf8')).hexdigest()
        if table1_md5 == table2_md5:
            result = {"result": True}
            return result
        result = {"result": False, "table1_md5": table1_md5, "table2_md5": table2_md5}
        return result
    ...
    def incrementalRowsContentCompare(self, get_data_sql1, get_data_sql2, get_primary_key_sql1,
        ...
        while True:
            ...
            if primary_key_bool:
                ...
                logger.info(f'切片{count}：行内容对比报告：{report}\n')
                continue
            indexes = DatabaseData(self.conn1, self.db1, self.table1).getIndexes(
                get_indexes_sql1)
            indexes_bool = ~indexes.isnull()['COLUMN_NAME'][0]
            if indexes_bool:
