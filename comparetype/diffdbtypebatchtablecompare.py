...
class DiffDbTypeBatchTableCompare():
    def __init__(self,conf_user,conf_db,conf_sql,batch_file):
        ...
        match self.target_db_type:
            case 'MYSQL':
                print(f'连接目标数据库MYSQL：{self.target_db_connect_info}')
                self.target_conn = DatabaseConnect(self.target_db_connect_info).connectMysql()
            case 'DB2':
                print(f'连接目标数据库DB2：{self.target_db_connect_info}')
                self.target_conn = DatabaseConnect(self.target_db_connect_info).connectDb2()
            case 'GaussDB':
                print(f'连接目标数据库GaussDB：{self.target_db_connect_info}')
    ...
    def fullDataCompare(self):  # 全量对比
        for _, line in enumerate(open(self.batch_file, 'r')):
            ...
            f.write(f'主键对比结果：{compare_primary_key_result[0]}\n')
            f.write(f'主键对比报告：{compare_primary_key_result[1]}\n')
            ...
            f.write(f'索引对比结果：{compare_indexes_result[0]}\n')
            f.write(f'索引对比报告：{compare_indexes_result[1]}\n')
