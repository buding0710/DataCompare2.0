...
class SameDbTypeTwoTableCompare():
    def __init__(self,conf_user,conf_db,conf_sql):
        ...
        self.comp = CompareDatabaseData(self.source_conn, self.source_db, self.source_table, self.target_conn,
                                        self.target_db, self.target_table)
        today = str(datetime.date.today())
        current_time = time.strftime('%H%M%S', time.localtime())
        f_path = './report/{to}/{li}_{ct}.txt'.format(to=today, ct=current_time,
                                                      li=self.source_table.rstrip(
                                                          '\n') + ',' + self.target_table.rstrip('\n'))
        self.f = open(f_path, 'a')

    def tableStructureCompare(self):
        logger.info(
            "************************************************表结构对比****************************************************")
        #表结构对比
        table_structure_sql = GetConfig(self.conf_sql, eval(self.source_db_connect)[1]).getItem('get_table_structure')
        compare_table_structure_result = self.comp.tableStructureCompare(table_structure_sql)
        self.f.write(f'表结构对比结果：{compare_table_structure_result[0]}\n')
        self.f.write(f'表结构对比报告：{compare_table_structure_result[1]}\n')
    ...
    def columnsCompare(self):
        ...
        compare_columns_num_result = self.comp.columnsNumCompare(source_db_columns_number_sql,target_db_columns_number_sql)
        ...
        compare_columns_name_result = self.comp.columnsNameCompare(source_db_columns_name_sql,target_db_columns_name_sql)
    ...
    def primaryKeyCompare(self):
        ...
        compare_primary_key_result = self.comp.columnsNameCompare(source_db_primary_key_sql,
    ...
    def indexesCompare(self):
        ...
        compare_indexes_result = self.comp.columnsNameCompare(source_db_indexes_sql,
    ...
    def rowsNumCompare(self):
        ...
        compare_rows_result = self.comp.rowsNumCompare(source_db_rows_sql,
                                                                                           target_db_rows_sql)
    ...
    def fullDataCompare(self):  # 全量对比
        ...
        compare_full_table_result = self.comp.fullTableMD5compare(source_db_data_sql, target_db_data_sql,
    ...
    def sampleDataCompare(self):  # 抽样对比
        ...
        join_columns = self.user_profile['join_columns']
        once_get_rows = int(self.user_profile['once_get_rows'])
        compare_rows_content_result = self.comp.sampleRowsContentCompare(source_db_get_slice_data_sql,target_db_get_slice_data_sql,source_db_primary_key_sql,target_db_primary_key_sql, source_db_indexes_sql, target_db_indexes_sql, source_db_columns_name_sql,target_db_columns_name_sql, sort_values,once_get_rows,join_columns,ratio)
    ...
    def incrementalDataCompare(self):  # 增量对比
        ...
        compare_full_table_result = self.comp.fullTableMD5compare(incremental_data_sql1, incremental_data_sql2,
                                                              source_db_primary_key_sql, target_db_primary_key_sql,
                                                              source_db_indexes_sql, target_db_indexes_sql,
                                                              source_db_columns_name_sql, target_db_columns_name_sql,
                                                              sort_values)
        ...
        if compare_full_table_result['result']:
            ...
            logger.info('全表数据MD5比对通过，终止执行！\n')
            return compare_full_table_result
        compare_rows_content_result = self.comp.incrementalRowsContentCompare(source_db_get_slice_data_sql, target_db_get_slice_data_sql,
                                                               source_db_primary_key_sql, target_db_primary_key_sql,
                                                               source_db_indexes_sql, target_db_indexes_sql,
                                                               source_db_columns_name_sql, target_db_columns_name_sql,
