...
class TwoFileCompare():
    def __init__(self,user_conf):
        ...
        f_path = './report/{to}/{sf}&{tf}_{ct}.txt'.format(to=today, ct=current_time,
                                                           sf=self.source_file_path.split('\\')[-1],
                                                           tf=self.target_file_path.split('\\')[-1].rstrip('\n'))
    ...
    def rowsNumCompare(self):
        ...
        logger.info(
            "************************************************行数对比****************************************************")
        # 行数对比
        compare_rows_result = CompareRows(Sort1, Sort2).rowsNumCompare()
    ...
    def fullDataCompare(self):#全量对比
        ...
        logger.info(
            '************************************************行内容对比完成****************************************************\n')
        self.f.write(f'行内容对比结果：{results}\n')
        self.f.write(f'行内容对比报告：{reports}\n')
        self.f.close()
