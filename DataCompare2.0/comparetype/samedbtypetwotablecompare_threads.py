...
from DataCompare.datacheck.databasedatacompare_threads import CompareDatabaseData
...
class SameDbTypeTwoTableCompare():
    ...
    def tableStructureCompare(self):
        ...
        compare_table_structure_result = CompareDatabaseData(self.source_conn,self.source_db,self.source_table,self.target_conn,self.target_db,self.target_table).tableStructureCompare(table_structure_sql)
        logger.info(f'表结构对比结果：{compare_table_structure_result}')
    ...
    def fullDataCompare(self):  # 全量对比
        ...
        compare_full_table_result = CompareDatabaseData(self.source_conn, self.source_db, self.source_table, self.target_conn, self.target_db,
                                                          self.target_table).fullTableMD5compare(source_db_data_sql, target_db_data_sql,
    ...
    def incrementalDataCompare(self):  # 增量对比
        ...
        compare_full_table_result = CompareDatabaseData(self.source_conn, self.source_db, self.source_table, self.target_conn,
                                       self.target_db,
                                       self.target_table).fullTableMD5compare(incremental_data_sql1, incremental_data_sql2,
                                                              source_db_primary_key_sql, target_db_primary_key_sql,
                                                              source_db_indexes_sql, target_db_indexes_sql,
                                                              source_db_columns_name_sql, target_db_columns_name_sql,
                                                              sort_values)
