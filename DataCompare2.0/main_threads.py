...
from DataCompare.comparetype.batchfilecompare_threads import BatchFileCompare
from DataCompare.comparetype.diffdbtypebatchtablecompare_threads import DiffDbTypeBatchTableCompare
from DataCompare.comparetype.diffdbtypetwotablecompare_threads import DiffDbTypeTwoTableCompare
from DataCompare.comparetype.samedbtypebatchtablecompare_threads import SameDbTypeBatchTableCompare
from DataCompare.comparetype.samedbtypetwotablecompare_threads import SameDbTypeTwoTableCompare
from DataCompare.comparetype.twofilecompare_threads import TwoFileCompare
...
if __name__ == '__main__':
    ...
    for _, line in enumerate(open(batch_file, 'r')):
        log_path = './log/{to}/{li}_{ct}.log'.format(to=today, ct=current_time,
                                                     li=line.split('\\')[-1].split('.')[0].rstrip('\n'))
        logger.info(f'批量文件对比logpath:{log_path}')
        logger.add(log_path, level='INFO')
        match compare_type:
            case 'full_compare':
                logger.info('开始全量对比，当前线程为：' + threading.current_thread().name)
