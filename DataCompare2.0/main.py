...
from DataCompare.comparetype.batchfilecompare import BatchFileCompare
from DataCompare.comparetype.diffdbtypebatchtablecompare import DiffDbTypeBatchTableCompare
from DataCompare.comparetype.diffdbtypetwotablecompare import DiffDbTypeTwoTableCompare
from DataCompare.comparetype.samedbtypebatchtablecompare import SameDbTypeBatchTableCompare
from DataCompare.comparetype.samedbtypetwotablecompare import SameDbTypeTwoTableCompare
from DataCompare.comparetype.twofilecompare import TwoFileCompare
...
if __name__ == '__main__':
    ...
    if not os.path.exists('./log/' + today):
        os.mkdir('./log/' + today)
    if not os.path.exists('./log/' + today + '/' + current_time):
        os.mkdir('./log/' + today + '/' + current_time)
    ...
    compare_type = user_profile.getItem('compare_type')

    #不同数据库类型批量表对比
    batch_table = r'D:\pythonProject\DataCompare\configuration\batchtablenames.txt'
    match compare_type:
        case 'full_compare':
			print('开始全量对比')
    ...
    log_path = './log/{to}/{ct}/{st}&{tt}.log'.format(to=today, ct=current_time, st=source_table,tt=target_table.replace('\n', '').replace('\r', ''))
    ...
    source_table = user_profile['source_table']
    target_table = user_profile['target_table']
    log_path = './log/{to}/{ct}/{st}&{tt}.log'.format(to=today, ct=current_time, st=source_table,
    ...
    log_path = './log/{to}/{ct}/{sf}&{tf}.log'.format(to=today, ct=current_time, sf=source_file,
