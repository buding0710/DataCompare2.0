...
class CompareDatabaseData:
    ...
    def slice_parse(self, join_columns, slice, get_primary_key_sql1,get_indexes_sql1):
        with lock:
            logger.info('切片行内容对比开始...')
            logger.info(f"{time.time()}》》》》》》》》》》》》》》》》》》》》》》》》》》》》》》》》》》》》协程：{gevent.getcurrent()}, {random.randint(1, 100)}")
            ...
            if not l and not r:
                result = {slice[0]: True}
                logger.info('切片slice[0]行内容对比结果通过！')
                return result
            l_index = [md5_list1.index(i) for i in l]
            r_index = [md5_list2.index(i) for i in r]
            ...
            if eval(join_columns):
                ...
                logger.info(f'切片slice[0]行内容对比结果：{res.report()}')
                return result
